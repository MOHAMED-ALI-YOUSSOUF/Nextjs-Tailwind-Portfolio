...
const projects = [
    {
      ...
      description: "A personal portfolio website showcasing the work of Saleh Ismael, featuring interactive elements and a modern design.",
    ...
    },
    {
      id: 2,
      title: "Djibstudy",
      description: "An educational platform for Djibouti student, providing resources for students.",
    ...
    },
    {
      id: 3,
      title: "IT Mastery",
      description: "Educational platform create using React.js & Bootstrap with a team",
    ...
    },
    {
      ...
      description: "A website for the Afar Cultural Association, aimed at promoting Afar culture and connecting the diaspora.",
  ...
  const FeaturedProject = ({ type, title, summary, img, link, width, height }) => {
    return (
      <article className='w-full flex items-center  justify-between rounded-3xl border border-solid border-dark bg-light shadow-2xl p-12 relative rounded-br-2xl dark:bg-dark dark:border-light lg:flex-col lg:p-8 xs:rounded-2xl sx:rounded-br-3xl xs:p-4'>
        ...
        <FramerImage src={img} alt={title} width={width} height={height} whileHover={{scale:1.05}}  
          transition={{duration:0.4}} className='w-full h-auto' priority sizes="(max-width:768px) 100vw, (max-width:1200px) 50vw, 50vw" />
        </Link>
        <div className='w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6'>
          <span className='text-primary font-medium text-xl dark:text-primaryDark xs:text-base'>{type}</span>
          <Link href={link} target='_blank' className='hover:underline underline-offset-2'>
            <h2 className='my-2 w-full text-left text-4xl font-bold dark:text-light text-dark sm:text-sm'>{title}</h2>
          </Link>
          <p className='my-2 font-medium text-dark dark:text-light sm-text-sm'>{summary}</p>
          <div className='mt-2 flex items-center'>
            {/* <Link href={github} target='_blank' className='w-10'><GithubIcon /></Link> */}
            <Link href={link} target='_blank'  className="ml-4 rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold dark:bg-light dark:text-dark hover:dark:bg-dark hover:dark:text-light border border-solid hover:border-dark hover:bg-light hover:text-dark transition duration-150 ease-in-out"
    >Visit Project</Link>
          </div>
        </div>
      </article>
  ...
  const Project = ({ type, title, summary, img, link, width, height }) => {
    return (
      ...
      className='w-full flex flex-col items-center justify-between rounded-3xl border border-solid border-dark bg-light p-6 relative dark:bg-dark dark:border-light xs:p-4'
    >
        ...
        <div className='w-full flex flex-col items-start justify-between pt-5 pl-6'>
          ...
          <Link href={link} target='_blank' className=' hover:underline underline-offset-2'>
            <h2 className='my-2 w-full text-left text-4xl font-bold dark:text-light text-dark lg:text-2xl'>{title}</h2>
          </Link>
          <p className='my-2 font-medium text-dark dark:text-light xs:text-base'>{summary}</p>
          <div className='mt-2 flex items-center'>
            {/* <Link href={github} target='_blank' className='w-10'><GithubIcon /></Link> */}
            <Link href={link} target='_blank' className='ml-4 rounded-lg bg-dark text-light p-2 px-6 text-lg font-semibold dark:bg-light dark:text-dark hover:dark:bg-dark hover:dark:text-light border border-solid hover:border-dark hover:bg-light hover:text-dark transition duration-150 ease-in-out '>Visit </Link>
          </div>
        </div>
      </article>
    );
